...
function isTriple(position, value, countX, countO) {
  ...
  if (countX.includes(3) || countO.includes(3)) {
    return true;
  }
  return false;
}
...
function turns(board, player1, player2, countX, countO) {
  ...
  for (let count = 0; count < 9; count++) {
    const symbol = count % 2 === 0 ? PLY1_SYL : PLY2_SYL;
    const player = PLY1_SYL === symbol ? player1 : player2;
    console.log(player);
    ...
    if (isTriple(position, symbol, countX, countO)) {
      win(position, symbol, player1, player2, countX, countO);
      return;
    }
  }
  win();
